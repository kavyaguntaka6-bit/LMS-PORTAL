...
export const RatingStars: React.FC<RatingStarsProps> = ({
  ...
  for (let i = 1; i <= max; i++) {
    if (i <= Math.floor(rating)) {
      ...
      stars.push(<Star key={i} className={clsx(iconSizes[size], 'text-gray-200 fill-gray-100')} />);
    }
  }
  ...
  return (
    <div className={clsx('inline-flex items-center gap-1.5', className)}>
      ...
      {showNumber && (
        <span className="text-xs font-semibold text-tyc-text">
          {rating.toFixed(1)}
        </span>
      )}
      {reviewsCount !== undefined && (
        <span className="text-xs text-tyc-muted">({reviewsCount.toLocaleString()})</span>
      )}
    </div>
  );
};
